...
def negative_assert_symbol(first_name):
    ...
    #Проверяет, что статус-код равен 400
    assert user_response.status_code == 400

    #Проверяет, что в теле ответа атрибут code = 400
    assert user_response.json()["code"] == 400

    #Проверяет, что в теле ответа атрибут message = "Имя пользователя введено некорректно. Имя может содержать только русские или латинские буквы, длина должна быть не менее 2 и не более 15 символов"
...
def test_create_user_russian_letter_in_first_name_get_success_response():
    positive_assert("евгеНий")

# Тест 7. Ошибка
def test_create_user_has_space_in_first_name_get_error_response():
    negative_assert_symbol("AA AA AA")
...
# Тест 9. Ошибка
def test_create_user_has_number_symbol_in_first_name_get_error_response():
